    ...
    public void Apply(OpenApiRequestBody requestBody, RequestBodyFilterContext context)
    {
        ...
        if (context.BodyParameterDescription.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase)
            || context.BodyParameterDescription.Type
                .GetProperties().Any(p => p.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase)))
            requestBody.Description =
                "IMPORTANT: be sure to always use a strong password and store it in a secure location!";
    }
}